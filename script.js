...
const examStyleMapConfig = [ // Your black and white + state boundaries style
    { featureType: "all", elementType: "labels", stylers: [{ visibility: "off" }] },
    { featureType: "landscape", elementType: "geometry", stylers: [{ color: "#ffffff" }] },
    { featureType: "water", elementType: "geometry", stylers: [{ color: "#0E87CC" }] },
    { featureType: "administrative", elementType: "geometry.stroke", stylers: [{ visibility: "on" }, { color: "#000000" }, { weight: 0.8 }] },
    { featureType: "administrative.country", elementType: "geometry.stroke", stylers: [{ color: "#000000" }, { weight: 1.5 }, { visibility: "on" }] },
    { featureType: "administrative.province", elementType: "geometry.stroke", stylers: [{ color: "#000000" }, { weight: 0.8 }, { visibility: "on" }] }, // State boundaries
    ...
    // ];
const examStyleMapNoStateBoundariesConfig = [ // Black and white, NO state boundaries
    { featureType: "all", elementType: "labels", stylers: [{ visibility: "off" }] },
    { featureType: "landscape", elementType: "geometry", stylers: [{ color: "#ffffff" }] },
    { featureType: "water", elementType: "geometry", stylers: [{ color: "#dddddd" }] },
    { featureType: "administrative", elementType: "geometry.stroke", stylers: [{ visibility: "on" }, { color: "#000000" }, { weight: 0.8 }] },
    { featureType: "administrative.country", elementType: "geometry.stroke", stylers: [{ color: "#000000" }, { weight: 1.5 }, { visibility: "on" }] },
    { featureType: "administrative.province", elementType: "geometry.stroke", stylers: [{ color: "#ffffff" }, { visibility: "on" }] }, // Hide state boundaries
    { featureType: "road", stylers: [{ visibility: "off" }] },
    { featureType: "poi", stylers: [{ visibility: "off" }] },
    { featureType: "transit", stylers: [{ visibility: "off" }] }
];
...
function initApp() {
    ...
    startRandomQuizBtnSidebar = document.getElementById('startRandomQuizBtnSidebar');
    ...
    returnToSectionsFromStudyBtn = document.getElementById('returnToSectionsFromStudyBtn');
    ...
    toggleStateBoundariesCheckbox = document.getElementById('toggleStateBoundaries');
...
function showQuestionPaneView(viewName) {
    ...
    } else if (viewName === 'studyMode') { // Add this case
...
function populateSectionsSidebar() {
    ...
    const createSectionEntry = (containerDiv, subCategoryName, items) => {
        ...
        viewAllBtn.addEventListener('click', () => showAllAnswersForSection(subCategoryName, items));
    ...
    for (const subCategory in categories.History) {
        createSectionEntry(historySectionsDiv, subCategory, categories.History[subCategory]);
    }

    geographySectionsDiv.innerHTML = '<h4 class="nav-title-full">Geography</h4>';
    for (const subCategory in categories.Geography) {
        createSectionEntry(geographySectionsDiv, subCategory, categories.Geography[subCategory]);
    }
}
...
function setupSettingsListeners() {
    ...
    toggleSignificanceCheckbox.addEventListener('change', (e) => {
     ...
     toggleStudyLabelsCheckbox.addEventListener('change', (e) => { // New Listener
...
function updateStudyModeMarkersLabels() {
    if (studyModeContentEl.classList.contains('active-view')) { // Only if study mode is active
        studyModeMarkers.forEach(marker => {
            if (showStudyLabels) {
                ...
                if (marker.customLabelText) { // Check if we stored it
                     marker.setLabel({
...
function setupGlobalEventListeners() {
    ...
    nextQuestionBtn.addEventListener('click', () => { // From previous script
    ...
    returnToSectionsBtn.addEventListener('click', () => {
        ...
        // Potentially reset map zoom/pan here
         if (window.innerWidth > 768 && leftSidebarPane.classList.contains('collapsed')) {
...
function startQuiz(quizTitle, items) {
    ...
    // --- Reset Map View ---
    if (map) { // Ensure map object exists
...
function loadNextQuestion() {
    if (currentQuestionIndex === 0) { // Only fully clear if it's the first question of a new quiz attempt
    ...
    userMarker = null; correctLocationMarker = null; mapOverlayMarker = null;
    ...
    if (currentQuestionIndex < currentQuizItems.length) {
        ...
        hintTextEl.textContent = currentQuestion.hint ? `${currentQuestion.hint}` : "";
        ...
        if (currentQuestion.type === 'point') {
            ...
            if (currentQuestion.mapMarkerInfo &&
                ...
                mapOverlayMarker = new google.maps.marker.AdvancedMarkerElement({
                    position: { lat: currentQuestion.mapMarkerInfo.lat, lng: currentQuestion.mapMarkerInfo.lng },
                    map: map,
                    content: regionMarkerLabelElement,
                    title: `Region ${currentQuestion.mapMarkerInfo.label}: ${currentQuestion.targetAreaDescription || currentQuestion.name}`, // Tooltip
                ...
                map.panTo({ lat: currentQuestion.mapMarkerInfo.lat, lng: currentQuestion.mapMarkerInfo.lng });
                map.setZoom(6); // Adjust zoom as needed to show context

            } else {
                // Fallback if mapMarkerInfo is incomplete for a map marker
                console.warn("MapMarkerInfo for identify_region_mcq is incomplete. No map marker shown for:", currentQuestion.name);
...
function handleMapClick(event) { // Similar to before
    if (!quizContentAreaEl.classList.contains('active-view') || // Check if quiz content is active
        !currentQuestion || currentQuestion.type !== 'point' ||
        (submitAnswerBtn.disabled === false && nextQuestionBtn.style.display !== 'none')) {
        return;
    }
    if (userMarker) userMarker.setMap(null);
    userMarker = new google.maps.Marker({
        position: event.latLng, map: map, draggable: true,
        icon: { url: "http://maps.google.com/mapfiles/ms/icons/blue-dot.png" }
    });
    submitAnswerBtn.disabled = false;
}

function handleMcqOptionSelect(buttonEl, optionText) { // Similar to before
...
function handleSubmitAnswer() { // Similar to before
    ...
    if (currentQuestion.type === 'point') {
        if (!userMarker) { alert("Please place a pin!"); submitAnswerBtn.disabled = false; return; }
        const userPos = userMarker.getPosition();
        const correctPos = new google.maps.LatLng(currentQuestion.lat, currentQuestion.lng);
        resultInfo.distance = google.maps.geometry.spherical.computeDistanceBetween(userPos, correctPos) / 1000;
        isCorrect = resultInfo.distance <= ACCEPTABLE_POINT_RADIUS_KM;
        if (userMarker) userMarker.setDraggable(false);
    } else if (currentQuestion.type === 'identify_region_mcq') {
        if (!selectedMcqOption) { alert("Please select an option!"); submitAnswerBtn.disabled = false; return; }
...
function showFeedback(isCorrect, resultInfo) { // Similar, but applies visibility setting
    ...
    if (currentQuestion.type === 'point') {
        ...
        correctLocationMarker = new google.maps.Marker({
            position: { lat: currentQuestion.lat, lng: currentQuestion.lng }, map: map,
    ...
    significanceTextEl.textContent = currentQuestion.significance || "Significance not available.";
...
function showAllAnswersForSection(sectionName, items) {
    ...
    items.forEach(item => {
        ...
        card.addEventListener('click', () => {
            if ((item.type === 'point' && item.lat && item.lng) ||
                (item.type === 'identify_region_mcq' && item.mapMarkerInfo && item.mapMarkerInfo.lat && item.mapMarkerInfo.lng)) {
                let targetLat = item.type === 'point' ? item.lat : item.mapMarkerInfo.lat;
                let targetLng = item.type === 'point' ? item.lng : item.mapMarkerInfo.lng;
        ...
        let markerLat, markerLng, markerLabelTextForDisplay, markerTitle, actualLabelTextForStorage;
        ...
        } else if (item.type === 'identify_region_mcq' && item.mapMarkerInfo && item.mapMarkerInfo.lat && item.mapMarkerInfo.lng) {
        ...
        markerLabelTextForDisplay = showStudyLabels ? actualLabelTextForStorage : null;
        ...
        if (markerLat !== undefined && markerLng !== undefined) {
            ...
            if (markerLabelTextForDisplay) { // Only add label object if text is not null
                markerOptions.label = {
    ...
    if (studyModeMarkers.length > 0) {
        ...
        google.maps.event.addListenerOnce(map, 'bounds_changed', function() {
            if (this.getZoom() > 10 && studyModeMarkers.length <=3 ) { // Don't zoom in too much for few markers
                 this.setZoom(8);
            } else if (this.getZoom() > 12) { // General max zoom after fitBounds
                this.setZoom(12);
            }
        });
    ...
    if (window.innerWidth > 768 && !leftSidebarPane.classList.contains('collapsed')) {
        leftSidebarPane.classList.add('collapsed');
        updateSidebarToggleIcon();
    } else if (window.innerWidth <= 768 && leftSidebarPane.classList.contains('mobile-sidebar-open')) {
        leftSidebarPane.classList.remove('mobile-sidebar-open');
    }
}
    ...
    if (useColorfulMap) {
        ...
        if (showStateBoundaries) {
            ...
            return examStyleMapConfig;
                        ...
                        console.log("Applying examStyleMapNoStateBoundariesConfig (NO state boundaries)");
...
function applyMapStyle() {
    if (map) { // Ensure map object exists
        ...
        console.log("Map style applied:", newStyle ? "Custom Style" : "Default Google Style");    }
}
...
function createMap() {
    const mapOptions = {
        ...
        zoom: 5,
        minZoom: 4, maxZoom: 8,
        restriction: { latLngBounds: { north: 37.0, south: 7.0, west: 67.0, east: 98.0 }, strictBounds: false },
        mapTypeControl: false, streetViewControl: false, fullscreenControl: false, zoomControl: true,
    ...
    } else if (studyModeContentEl.classList.contains('active-view') && currentStudyingSectionItems.length > 0) {
        // Re-render all study mode markers (this will call clearAllMapMarkers first)
        showAllAnswersForSection(currentStudyingSectionName, currentStudyingSectionItems, false); // false to prevent sidebar collapse
    }
...
function conditionallyRenderMcqMarkerForCurrentQuestion() {
    ...
    if (currentQuestion && currentQuestion.type === 'identify_region_mcq' &&
        currentQuestion.mapMarkerInfo && currentQuestion.mapMarkerInfo.label &&
        ...
        mapOverlayMarker = new google.maps.marker.AdvancedMarkerElement({
            ...
            map: map,
            content: regionMarkerLabelElement,
            title: `Region ${currentQuestion.mapMarkerInfo.label}: ${currentQuestion.targetAreaDescription || currentQuestion.name}`,
        });
    }
}
